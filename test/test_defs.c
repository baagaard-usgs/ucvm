...
int
test_assert_double(double val1,
                   double val2) {
...
int
test_assert_string(const char* val1,
                   const char * val2) {
...
int
test_assert_file(const char *file1,
                 const char *file2) {
    FILE *fp1, *fp2;
...
/* Test execution */
int
test_run_suite(test_suite_t *suite) {
...
/* Run UCVM query tool */
int
run_ucvm_query(const char *bindir,
               const char *conf,
               const char *model,
               const char *map,
               const char *infile,
    ...
    sprintf(cmd, "%s/ucvm_query -m %s -p %s -c gd -f %s < %s > %s", bindir, model, map, conf, infile, outfile);
    printf("Running cmd: %s\n", cmd);
    if (system(cmd) < 0) {
        printf("FAIL: Error occurred while running ucvm_query command.");
        return 1;
    }
...
int
get_test_model(ucvm_model_t *m) {
    m->mtype = UCVM_MODEL_CRUSTAL;
    m->initialize = test_model_init;
    m->finalize = test_model_finalize;
    m->set_parameter = test_model_setparam;
    m->get_version = test_model_version;
    m->get_label = test_model_label;
