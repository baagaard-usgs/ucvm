...
/* Init flag */
int ucvm_cmuetree_create_flag = 0;
...
/* Create Cmuetree */
int
ucvm_cmuetree_model_create(int id,
                           const char *lib_dir,
...
int
ucvm_cmuetree_model_initialize() {
    return (UCVM_CODE_SUCCESS);
}
...
/* Finalize CMU etree */
int
ucvm_cmuetree_model_finalize() {
    if (ucvm_cmuetree_create_flag == 0) {
        fprintf(stderr, "Model not created\n");
        return (UCVM_CODE_ERROR);
    }
...
/* Version CMU etree */
int
ucvm_cmuetree_model_version(int id,
                            char *ver,
                            int len) {
...
/* Label Cmuetree */
int
ucvm_cmuetree_model_label(int id,
                          char *lab,
                          int len) {
    if (id != ucvm_cmuetree_id) {
        fprintf(stderr, "Invalid model id\n");
...
/* Query Cmuetree */
int
ucvm_cmuetree_model_query(int id,
                          ucvm_ctype_t cmode,
                          int n,
                          ucvm_point_t *pnt,
                          ucvm_data_t *data,
...
int
ucvm_cmuetree_get_model(ucvm_model_t *m) {
