 ...
 * by ucvm_model_t */
int ucvm_add_model_bylist(const char *list);

int ucvm_add_model_bylabel(const char *label);

int ucvm_add_model_bytype(ucvm_model_t *m,
                          ucvm_modelconf_t *mconf);

/* Associate specific interp func with GTL model, by label
 * or by ucvm_ifunc_t */
int ucvm_assoc_ifunc(const char *mlabel,
                     const char *ilabel);

int ucvm_assoc_user_ifunc(const char *mlabel,
                          ucvm_ifunc_t *ifunc);
...
int ucvm_model_label(int m,
                     char *label,
                     int len);

/* Get label for an interpolation function */
int ucvm_ifunc_label(int f,
                     char *label,
                     int len);

/* Get version for a model */
int ucvm_model_version(int m,
                       char *ver,
                       int len);
...
int ucvm_get_resources(ucvm_resource_t *res,
                       int *len);
