...
#define MODEL_FILENAME "1d/1dgtl.config"

/* Create flag */
int ucvm_1dgtl_create_flag = 0;
...
double
ucvm_1dgtl_scec_vp(double depth) {
    int i;
    double vp;
    double depth_ratio;
    double vp_range;
...
ucvm_1dgtl_model_create(int m,
    ...
    if (ucvm_1dgtl_create_flag) {
        fprintf(stderr, "Model %s is already created\n", conf->label);
        return (UCVM_CODE_ERROR);
    }

    if ((models_path == NULL) || (strlen(models_path) == 0)) {
        fprintf(stderr, "Empty models path when creating model %s\n", conf->label);
        return (UCVM_CODE_ERROR);
    }
...
/* Finalize 1DGTL */
int
ucvm_1dgtl_model_finalize() {
    ucvm_1dgtl_z_dim = 0;
    ucvm_1dgtl_create_flag = 0;
...
/* Version 1DGTL */
int
ucvm_1dgtl_model_version(int id,
                         char *ver,
                         int len) {
    if (id != ucvm_1dgtl_id) {
...
/* Set_parameter 1DGTL */
int
ucvm_1dgtl_model_set_parameter(int id,
                               const char *name,
                               const char *value) {
    if (id != ucvm_1dgtl_id) {
        fprintf(stderr, "Invalid model id\n");
...
int
ucvm_1dgtl_get_model(ucvm_model_t *m) {
    m->mtype = UCVM_MODEL_GTL;
    m->create = ucvm_1dgtl_model_create;
    m->initialize = ucvm_1dgtl_model_initialize;
    m->finalize = ucvm_1dgtl_model_finalize;
    m->set_parameter = ucvm_1dgtl_model_set_parameter;
    m->get_version = ucvm_1dgtl_model_version;
    m->get_label = ucvm_1dgtl_model_label;
